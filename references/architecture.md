...
7. `trip-data.json`, HTML from `html_renderer.py`, optional PDF, and
   `manifest.json` are generated in a same-filesystem staging directory.
8. `manifest_contract.py` provides the machine-readable run summary.
9. `trip_pipeline.py` publishes the staged generated-file set with rollback of
